...
import { planScenes } from "./timing.ts";
import { narrationText, synthesizeVoice } from "./tts.ts";
import type { ProduceOptions, VisualScene } from "./types.ts";
...
export async function produce(options: ProduceOptions) {
  ...
  const scenes: VisualScene[] = planScenes(script, voice.duration, voice.words);
  const { duration } = await renderVideo({
    script,
    wavPath: files.audio,
    mp4Path: files.video,
    assPath: files.captions,
    scenesPath: files.scenes,
    aspect,
    words: voice.words,
    scenes,
  });
